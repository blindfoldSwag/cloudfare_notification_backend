...
async function chooseScheduledType(env, device) {
  ...
  if (boolEnabled(prefs.boss) && snapshot.bossActionAvailable && !wasSent("boss_ready") && local.hour >= 12 && local.hour < 20) {
    return "boss_ready";
  }

  if (boolEnabled(prefs.morning) && !wasSent("daily_briefing") && local.hour >= 6 && local.hour < 11) {
